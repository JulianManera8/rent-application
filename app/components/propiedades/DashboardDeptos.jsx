...
export default function DashboardDeptos({ searchTerm, filteredDepartamentos, loadingDeptos }) {
      ...
      if (userLoged_id && dataGrupos?.length > 0) {
        try {
          ...
          if (dataRoles) {
            ...
            const fetchedDeptosAddedByShared = await getAccessdeptos(usersShared_ids);
            setDeptosAddedByShared(fetchedDeptosAddedByShared);
          }
        } catch (err) {
  ...
  function agruparUsersAccessPorGrupo(dataGruposAccess) {
    return dataGruposAccess.map(user => ({
      grupoId: user.grupo_id,
      user_id_shared: user.user_id_access,
    }));
  }

  const allDepartamentos = [...filteredDepartamentos, ...deptosAddedByShared];
  ...
  return (
    <div className='w-full py-10'>
                  ...
                  <div className="overflow-x-auto">
                    <Table className="border-b-2 w-full">
                      ...
                      <TableBody>
                        ...
                        ) : allDepartamentos.filter(depto => depto.grupo_id === grupo.id).length === 0 ? (
      ...
      showSkeleton ? Array.from({ length: 2 }).map((_, index) => (
        <div key={`skeleton-${index}`} className="flex flex-col md:flex-row w-full justify-between items-start md:items-center relative mb-8 md:mb-12">
          <div className="flex justify-between w-full items-center mb-4 md:mb-1">
            <Skeleton className="h-10 w-full md:w-1/2 bg-gray-100" />
          </div>
